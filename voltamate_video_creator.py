...
class reddit_grabber():
    def __init__(self):
        ...
        self.voices = ["Joanna","Amy","Kendra", "Salli", "Ivy", "Joey", "Matthew", "Emma","Arthur"]
    ...
    def save_polly_mp3_file(self,spoken_text,video_part):
        ...
        try:
            response = polly.synthesize_speech(Text=spoken_text, OutputFormat="mp3",
                                                VoiceId=random.choice(self.voices), Engine="neural")
    ...
    def create_intro(self):
        ...
        print(f"Intro Has Been Created Successfully!")
    ...
    def create_video(self):
        ...
        print(f"Main Has Been Created Successfully!")
    ...
    def create_outro(self):
        ...
        audio_clip = AudioFileClip(self.mp3_outro_name)
        duration = audio_clip.duration

        image_clip = ImageClip(self.image_name_cropped).set_duration(duration)
        screensize = (576,800)

        credits = (TextClip("Subscribe to watch new videos every day!", color='red',stroke_color= 'white',
                            stroke_width = 10,
                font="Berlin-Sans-FB-Bold", kerning=-2, interline=-1, size = 
        screensize, method='caption')
            .set_duration(duration)
        ...
        self.outro_name = self.time_id+"_outro.mp4"
        video = video.write_videofile(self.outro_name, fps=24)
        print(f"Outro Has Been Created Successfully!")

    def merge_video(self):
        intro = VideoFileClip(self.intro_name)
        main = VideoFileClip(self.main_name)
        outro = VideoFileClip(self.outro_name)
        ...
        with open(self.video_name+'.txt', 'w') as f:
            f.write(str(reddit_content.linked_media_url)+'\n'+str(reddit_content.title))     


#%%

def video_pipeline(k):
    ...
    reddit_content.save_polly_mp3_file("Hey, like and Subscribe for new videos every day!","outro")
